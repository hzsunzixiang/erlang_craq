"""
Main entry point for CRAQ Paper Reading MCP Server.
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="craq-paper-mcp",
        description="CRAQ Paper Reading MCP Server - A FastMCP 2.0 based server for reading CRAQ papers",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  craq-paper-mcp                    # Run server in stdio mode
  craq-paper-mcp --version          # Show version information
  uvx craq-paper-mcp                # Run via uvx

For more information, visit: https://github.com/craq/craq-paper-mcp
        """
    )
...
async def async_main(args) -> None:
    ...
    """
    if args.debug:
        print(f"Starting CRAQ Paper Reading MCP Server v{__version__} in debug mode", file=sys.stderr)
...
def main() -> NoReturn:
    """
    Main entry point for the CRAQ Paper Reading MCP server.
